...
export class View {
  ...
  public draw(): void {
    ...
    if (this.viewType == 0) {
      ...
      mat4.lookAt(this.modelview.peek(), vec3.fromValues(0, 100, 0), vec3.fromValues(0, 0, 0), vec3.fromValues(0, 1, 0));
    }
  ...
  /*
      public setLights(scenegraph: Scenegraph<VertexPNT>) {
        let i : number = 
        for (let n in scenegraph.getNodes()) {
          let curNode : SGNode = scenegraph.getNodes().get(n);
